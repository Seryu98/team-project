...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, device_info: str = "unknown"):
        """새 WebSocket 연결 시도"""
        ...
        if user_id in self.active_connections:
            ...
            try:
                ...
                await asyncio.sleep(1.0)  # ✅ 전송 안정화 대기 (기존 0.3 → 1.0초로 변경)
                try:
                    await old_socket.close(code=4001, reason="다른 기기에서 로그인됨")
        ...
        print(f"[SessionManager] 새로운 WebSocket 세션 등록: {user_id} ({device_info})")
...
import builtins
if not hasattr(builtins, "global_ws_manager"):
    builtins.global_ws_manager = ConnectionManager()

manager = builtins.global_ws_manager
