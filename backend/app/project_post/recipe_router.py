...
def to_dto(post: models.RecipePost) -> RecipePostResponse:
    return RecipePostResponse(
        ...
        leader_nickname=post.leader.nickname,
...
async def get_my_projects(
    status: Optional[str] = Query("ONGOING"),  # ONGOING, ENDED
    ...
    posts = (
        ...
        .join(PostMember)  # ✅ 수정
        .filter(
            PostMember.user_id == current_user.id,  # ✅ 수정
            models.RecipePost.project_status == status,
...
async def approve_application(
    ...
    db.add(PostMember(post_id=post_id, user_id=application.user_id, role="MEMBER"))  # ✅ 수정
    db.commit()
...
async def leave_post(
    ...
    membership = db.query(PostMember).filter(  # ✅ 수정
        PostMember.post_id == post_id,  # ✅ 수정
        PostMember.user_id == current_user.id  # ✅ 수정
    ).first()
