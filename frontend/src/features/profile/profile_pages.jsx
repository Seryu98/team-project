...
export default function ProfilePage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (userId) {
        // ✅ 비로그인자도 접근 가능 (공개 API)
        const res = await api.get(`/public/profiles/${userId}/projects`, config);
      ...
      // ✅ 상태 분류
      setOngoingProjects(allProjects.filter(p => p.status === "APPROVED" && p.project_status === "ONGOING"));
...
const fetchMyPosts = async () => {
  try {
    ...
    // ✅ 비로그인자도 접근 가능 (공개 API)
    const res = await api.get(`/board/user/${targetUserId}/posts`, config);
